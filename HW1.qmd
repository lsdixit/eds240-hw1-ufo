...
4.  **Describe what this data frame contains.**
    ...
    -   This data frame contains the number of UFO observations, total summed population, number of observations per 10,000 population

5.  **What does `opacity_val` represent, and why is it calculated?**

    -   opacity_val represents how light or dark the final plot colors will be as it corresponds to the number/10k value. A higher number/10k will be a darker green.
...
11. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
    ...
    -   `Paste0()` creates string expressions by separating elements with quotes, with any r functionality not in quotes and separates by commas, and combining elements into the final expression. While `glue()` is similar, it creates string expressions within a single quoted expression with r functionality or elements within `{}` brackets, referred to as string interpolation. String interpolation methods are especially useful for complicated HTML code, as the author used here, and tends to yield more readable string construction as compared to paste0().
...
14. **This plot includes one-letter labels for each day of the week. How is this accomplished when week days are written using their three-letter abbreviations (e.g. `Mon`, `Tue`) in the `df_day_hour` data frame?**
    ...
    -   The author uses `str_sub` to indicate which string to pull the letters from ("wday") and to start at the 1st character (start = 1) and to end at the 1st character (end = 1), pulling out only the first letter.
...
15. **What role do the `ylim()` and `xlim()` functions play in shaping a ggplot, and how do they change the visual layout of this particular plot? To better understand their effect, try rerunning the code with each of these lines commented out and observe how the plot’s spacing and composition change.**
    ...
    -   `ylim()` adjusts how close the hour labels are to the circular graph, larger values makes the labels further away. `xlim()` adjusts how close the bar for 11pm and 12am are, essentially how much extra space the graph has. A larger xlim opens up the circle more.
...
16. **Why do you think the author chose to convert these text elements (and also in `plot_ufo`, below!) into ggplot objects (you may consider returning to this question after you've worked your way through all of the code)?**
    ...
    -   By making each element into ggplot objects, the author can first make sure each element does not cause errors on their own and looks accurate to what the author wants them to look like visually. By not making separate objects, there is a higher chance the final visualization code could throw and error and not render. The final visualization can then be assembled with each working element, which also leads to cleaner final code that is easy to follow and understand. With each element as a separate object, the author can also easily manipulate where each graph is assembled on the infographic.
...
18. **Explain how `plot_final` is assembled. What do you think is the most challenging aspect of arranging all components into a single plot?**
    ...
    -   Each individual element (each quote and plot is separate) is individually added to the plot base which has the title and captions. I think the most challenging aspect is likely adjusting the location of each element on the base plot with the numbers between 0 and 1 such that it looks correct. 
...
21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?**
    ...
    -   I find the graph describing the most common reported shapes to be the most intuitive to understand. I also really enjoy the simplistic and fun color theme! It really leans into the sci-fi/alien aesthetic while still being mostly easy to read and look at.

22. **Describe two elements of this piece that you feel could be better presented in a different way. Why?**

    -   I think the final plot depicting when the most sighting were observed by hour and day could be better depicted. It's not entirely intuitive at first glance what is being looked at since there is no title. It also becomes a bit difficult to read the darker green over the black background, making the darkest shade a bit lighter would help.
